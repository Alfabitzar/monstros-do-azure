...
var exit = false;
const string SERVICEBUS_TOPIC = "news";
...
var sender = client.CreateSender(SERVICEBUS_TOPIC);
...
var producer = new Task(async () =>
{
    ...
    while (!exit)
    {
        var type = (NEWSTYPE)random.NextInt64(0, 3);
        var information = $"News {Guid.NewGuid()} of Type {type} created at {DateTime.Now:yyyy-MM-dd HH:mm:ss}";
